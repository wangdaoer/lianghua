"""Research-only focus pool for strong-gain A-share candidates."""
...
def _content_sha256(path: Path | None, fallback: Any = None) -> str:
    ...
    encoded = json.dumps(fallback, ensure_ascii=False, sort_keys=True, default=str).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def _momentum_input_fingerprint(
    result: MarketSnapshotLoadResult,
    *,
    as_of: Any,
    ...
    excluded_codes: set[str] | list[str] | tuple[str, ...] | None,
) -> str:
    payload = {
        "version": 1,
        ...
        "source_sha256": _content_sha256(result.source_path, result.rows),
        "input_row_count": len(result.rows),
        "strong_gain_threshold_pct": float(strong_gain_threshold_pct),
        "board_scope": board_scope,
        "target_horizon": int(target_horizon),
        "outcome_summary_sha256": _content_sha256(outcome_summary_path),
        "name_map_sha256": _content_sha256(name_map_path),
        "excluded_codes": sorted({_clean_code(code) for code in (excluded_codes or []) if _clean_code(code)}),
    }
...
def _build_outcome_prior_lookup(summary: pd.DataFrame) -> dict[tuple[str, str, str], dict[str, Any]]:
    ...
    for row in summary.to_dict(orient="records"):
        ...
        # find prior evidence for strong-gain candidates.
...
def _research_priority(change_pct: float, capital_utilization_score: float) -> str:
    if change_pct >= 9.0:
        return "medium"
    return "watch"


def _strong_gain_label(threshold_pct: float) -> str:
    return f"strong_gain_{float(threshold_pct):g}"
...
def build_momentum_focus_candidates(
    ...
    excluded_codes: set[str] | list[str] | tuple[str, ...] | None = None,
    ...
    excluded_by_review_required = 0
    normalized_excluded_codes = {
        code for value in (excluded_codes or ()) if (code := _clean_code(value))
    }
    ...
    volume_positive_count = sum(_as_float(row.get("volume")) > 0 for row in rows)
    turnover_positive_count = sum(_as_float(row.get("turnover")) > 0 for row in rows)
    volume_coverage_ratio = volume_positive_count / input_row_count if input_row_count else 0.0
    turnover_coverage_ratio = turnover_positive_count / input_row_count if input_row_count else 0.0
    minimum_coverage = min(volume_coverage_ratio, turnover_coverage_ratio)
    if input_row_count == 0:
        market_data_quality_status = "unavailable"
    elif minimum_coverage >= 0.98:
        market_data_quality_status = "good"
    elif volume_positive_count == 0 and turnover_positive_count == 0:
        market_data_quality_status = "missing_volume_turnover"
    else:
        market_data_quality_status = "partial"
    ...
    for raw in rows:
        ...
        if not code or board == "unknown":
            malformed += 1
            continue
        if code in normalized_excluded_codes:
            excluded_by_review_required += 1
            continue
        ...
        signal_type = _strong_gain_label(strong_gain_threshold_pct)
        ...
        candidate_rows.append(
            {
                ...
                "research_priority": _research_priority(pct, capital_utilization_score),
    ...
    payload = {
        ...
        "volume_positive_count": int(volume_positive_count),
        "turnover_positive_count": int(turnover_positive_count),
        "volume_coverage_ratio": float(volume_coverage_ratio),
        "turnover_coverage_ratio": float(turnover_coverage_ratio),
        ...
        "excluded_by_review_required_count": int(excluded_by_review_required),
        "excluded_by_review_required_codes": sorted(normalized_excluded_codes),
        ...
        "research_objective": "Improve candidate-generation success rate, return, and capital utilization through >=5% strong-gain stock-focused research pools.",
        "broker_action": "none",
        "research_only": True,
    }
...
| 成交量有效行数/覆盖率 | {snapshot.get("volume_positive_count")} / {_pct(_as_float(snapshot.get("volume_coverage_ratio")) * 100.0)} |
| 成交额有效行数/覆盖率 | {snapshot.get("turnover_positive_count")} / {_pct(_as_float(snapshot.get("turnover_coverage_ratio")) * 100.0)} |
| 量额数据质量 | `{snapshot.get("market_data_quality_status")}` |
| 候选数量 | {snapshot.get("candidate_count")} |
| 涨停数量 | {snapshot.get("limit_up_count")} |
| 涨幅大于等于5%数量 | {snapshot.get("strong_gain_count")} |
| 非主板/创业板排除数量 | {snapshot.get("excluded_non_main_chinext_count")} |
| ST/退市排除数量 | {snapshot.get("excluded_special_treatment_count")} |
| 复核硬排除数量 | {snapshot.get("excluded_by_review_required_count")} |
...
def run_momentum_focus(
    ...
    requested_trade_date = str(snapshot_trade_date)[:10] if snapshot_trade_date is not None else None
    if result is None or (requested_trade_date and result.trade_date and result.trade_date != requested_trade_date):
    ...
    input_fingerprint = _momentum_input_fingerprint(
        result,
        as_of=as_of,
        strong_gain_threshold_pct=strong_gain_threshold_pct,
        board_scope=board_scope,
        target_horizon=target_horizon,
        outcome_summary_path=resolved_summary_path,
        name_map_path=resolved_name_map_path,
        excluded_codes=excluded_codes,
    )
    if candidates_path.exists() and snapshot_path.exists():
        try:
            existing_snapshot = json.loads(snapshot_path.read_text(encoding="utf-8"))
            cached_candidates = pd.read_csv(candidates_path, dtype={"code": str}, encoding="utf-8-sig")
        except (OSError, UnicodeDecodeError, json.JSONDecodeError, pd.errors.EmptyDataError, pd.errors.ParserError):
            existing_snapshot = None
            cached_candidates = pd.DataFrame()
        if isinstance(existing_snapshot, dict) and existing_snapshot.get("input_fingerprint") == input_fingerprint:
            return MomentumFocusResult(
                output_dir=resolved_output,
                candidates_path=candidates_path,
                snapshot_path=snapshot_path,
                report_path=report_path,
                snapshot={**existing_snapshot, "cache_hit": True},
    ...
    candidates_changed = write_bytes_if_changed(candidates_path, candidate_bytes)
    snapshot = {
    ...
    snapshot, snapshot_changed = publish_json_if_semantically_changed(snapshot_path, snapshot)
    if candidates_changed or snapshot_changed or not report_path.exists():
        write_text_if_changed(report_path, _render_report(snapshot, candidates), encoding="utf-8")
    return MomentumFocusResult(
        output_dir=resolved_output,
        candidates_path=candidates_path,
        snapshot_path=snapshot_path,
