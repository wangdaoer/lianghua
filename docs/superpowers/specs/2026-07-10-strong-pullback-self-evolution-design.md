...
1. 读取并标准化行情，只保留 `research_start` 至 `validation_end` 的数据进入研究阶段。
2. 用基准参数执行一次研究回测，分别计算训练期和验证期指标。
3. 按 `search_groups` 顺序运行。每个候选都从当前保留版本复制参数，再应用该候选所属组的覆盖值。
4. 每组完成后，先按硬门槛淘汰，再在合格候选中选择验证期年化收益最高者；稳健评分只用于收益相同或非常接近时的排序。
5. 候选达到晋级要求时成为下一组的当前版本；否则保留当前版本并记录本组回滚。
6. 所有搜索组结束后才打开测试期数据，只用初始基准和研究冠军各运行一次完整回测。
7. 生成测试比较和人工确认状态，不覆盖任何现有正式配置。
...
- 验证期：`validation_start` 至 `validation_end`，是自动晋级的唯一决策区间。
- 测试期：`test_start` 至 `test_end` 或数据最新日期，只在研究冠军锁定后使用。
- 研究试验调用的价格矩阵必须在 `validation_end` 处物理截断，不能只在完整回测结果上隐藏测试列。
- 最终测试运行可以使用测试期之前的历史作为指标预热，但任何测试期收益都不能反向参与候选搜索。
- 时间区间必须满足 `train_end < validation_start <= validation_end < test_start`。
...
`--data` 必填，避免静默选错历史面板。`--benchmark` 可选；未提供时使用全仓位市场暴露，并在报告中明确记录。`--asof-date` 默认取输入数据最新日期。`--resume <run_id>` 只复用配置哈希、数据证据和试验标识完全匹配的已完成试验。
