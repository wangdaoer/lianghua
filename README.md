...
The controlled default switch was then applied to the research CLI defaults, module defaults, and `scripts\run_daily_research_refresh.ps1`. The active research allocator default is now `outputs\portfolio_source_selection\main_chinext_portfolio_source_selection_validation6_v1`, and the active phase-2 component snapshot is `outputs\research\phase2_model_status_source_selection_default_20260614\phase2_components.csv`. The verification run `outputs\research\daily_pipeline_default_candidate_switched_20260614\daily_pipeline.md` completed with dashboard posture `core_base_watch_allocator_gate`, model audit `ok`, promotion decision `promote_candidate`, alert level `info`, and paper-account metrics matching the candidate: 49.47% total return, -9.58% max drawdown, and 1.165 Sharpe. The old quality-v2 allocator remains the rollback/baseline comparison path, not the active research default.
